...
const updateDatabase = async (answers) => {
  ...
  // If User chose to Add Role
    ...
    await databaseQuery(
      `INSERT INTO role (title, salary, department_id) VALUES ("${answers.newRole}", "${answers.newRoleSalary}", "${newRoleDepartment}")`
    );
  ...
  // If user chose to Add Employee
    ...
    if (answers.newEmployeeManager === "No Manager") {
      ...
      await databaseQuery(
        `INSERT INTO employee (first_name, last_name, role_id) VALUES ("${answers.newEmployeeFirstName}", "${answers.newEmployeeLastName}", ${newEmployeeRoleID});`
      );
    } else {
      const managerNameArray = answers.newEmployeeManager.split(" ");
      newEmployeeManagerID = await databaseQuery(
      ...
      await databaseQuery(
        `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ("${answers.newEmployeeFirstName}", "${answers.newEmployeeLastName}", ${newEmployeeRoleID}, ${newEmployeeManagerID});`
      );
    }
    console.log(
      `New Employee "${answers.newEmployeeFirstName} ${answers.newEmployeeLastName}" added.`
    );
  }
  // If user chose to Update Employee
  else if (answers.actionChoice === "Update an Employee Role") {
    // Work for role value
    let updateEmployeeRoleID = await databaseQuery(
      `SELECT id FROM role WHERE title = "${answers.updateEmployeeRole}";`
    );
    ...
    updateEmployeeID = await databaseQuery(
      `SELECT id FROM employee WHERE first_name = "${employeeNameArray[0]}" AND last_name = "${employeeNameArray[1]}"`
    );
    updateEmployeeID = updateEmployeeID[0].id;
    // Write update Employee role to database
    await databaseQuery(
      `UPDATE employee SET role_id = "${updateEmployeeRoleID}" WHERE id = "${updateEmployeeID}";`
    );
    console.log(
      `Role of Employee "${answers.updateEmployee}" updated to "${answers.updateEmployeeRole}".`
    );
  }
};
